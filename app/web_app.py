...
def answer_the_question(question, num_doc_to_retrieve, context_window = 20000):
    retriever = vectordb.as_retriever( search_kwargs={"k": num_doc_to_retrieve})
    ...
    response = ollama.chat(model='llama3.2:latest', messages=[
      {
        'role': 'user',
        'content': f'''Answer the question according to the context given if question and context are related:
                       Question: {question}.
                       Context: {context}
                        ''',
      },
...
if 'num_doc_to_retrieve' not in st.session_state:
    st.session_state['num_doc_to_retrieve'] = "1"
...
answer = st.text_input("Ask a video game news related question", key="question", on_change = answer_the_question,
              args = (st.session_state.question, int(st.session_state.num_doc_to_retrieve)))
